...
Following what is going on with Covid-19 is challenging. 
Trends in the number of cases can be difficult to interpret as testing varies between countries and over time. 
So while in South Korea it is possible to get tested if you have even relatively mild symptoms, or where exposed to someone who is ill, in the UK you won't be tested unless you need hospitalisation with severe symptoms. 
...
These figures should also be interpreted with caution, however, as they absolute number of deaths does not account for the vast differences in the population sizes of different countries. 
The one aspect of the trends that does not depend on the size of the population (at the start of the epidemic) is the growth rate: how much the number of deaths is changing over time. 
...
Graphs of the log number of deaths by country can be found in the media -- although many of the better onese are behind paywalls (e.g. Financial Times). 
It is interesting, and perhaps useful, to be able to produce these graphs oneself. 
Fortunately, Johns Hopkins University in Baltimore, US, publishes an updated table of cumulative deaths each day, for each country in the world. 
We can use these data to produce a graph of the log deaths in whatever countries we choose. 

It is perfectly possible to download the data and create graphs with Excel or in Google Sheets. 
However, a very popular software for the analysis of data is the open-source statistics platform `R`. 
`R` takes a little bit of learning, but is widely favoured over other (expensive) statistics platforms. 

In the following sections, I am going to explain how you can get started in `R` and, for your first project, produce a graph of Covid-19 deaths.
...
Alternatively, you can copy-paste *all* of the code in one go by going [here]. 
...
When `read.csv` loads in the data at the URL below, we assign this data to the name `data_raw` using the `<-` symbols. 
