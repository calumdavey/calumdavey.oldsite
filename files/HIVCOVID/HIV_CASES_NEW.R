...
# LOAD THE DATA:
# http://ghdx.healthdata.org/record/ihme-data/gbd-2015-hiv-incidence-prevalence-and-mortality-1980-2015
# https://ourworldindata.org/hiv-aids
  d <- read.csv("deaths-and-new-cases-of-hiv.csv", as.is = T)

# DATA MANAGEMENT 
countries <- c('South Africa', 'Zimbabwe', 'Botswana')  
  
# PLOTTING THE DATA
max_x <- 10000000
max_y <- max(d$New.infections.of.HIV.AIDS..new.cases.of.HIV.infection.)

  # install.packages('RColorBrewer')
  library(RColorBrewer)
  cols <- brewer.pal(length(countries), 'Dark2')
  plot(c(1,1), type = 'n', 
       log = 'xy', # axes on the log scale 
       bty = 'n', # no border around the plot 
       xlim = c(1000,max_x+8000000),
       ylim = c(5000,8e+5),
       axes = FALSE, 
       xlab='Total number of cases',
       ylab='Cases in the past year',
       cex.lab=.7)
  ...
  for (country in countries){
    plot_data <- d[d$Entity == country,]
    # Get the number total cases 
    plot_data$cum_c <- cumsum(plot_data$New.infections.of.HIV.AIDS..new.cases.of.HIV.infection.)
    
    # Plot the lines 
    lines(plot_data$cum_c, plot_data$New.infections.of.HIV.AIDS..new.cases.of.HIV.infection.,
          col=cols[which(countries==country)])

    # Add a labels
    text(x=plot_data$cum_c[nrow(plot_data)], 
         y=plot_data$New.infections.of.HIV.AIDS..new.cases.of.HIV.infection.[nrow(plot_data)],
         label=country, 
...
# Add titles
  mytitle = "Recent and total deaths from HIV"
...
#####################
  ...
  countries <- c('China', 'United Kingdom', 'Italy', 'US')  
